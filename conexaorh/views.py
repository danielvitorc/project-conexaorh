...
def preencher_excel_rd(rd):
    ...
    ws["Z52"] = rd.data_autorizacao_diretor.strftime("%d/%m/%Y") if rd.data_autorizacao_diretor else ""
    ws["B59"] = rd.data_autorizacao_presidente.strftime("%d/%m/%Y") if rd.data_autorizacao_presidente else ""
    ws["Z59"] = rd.data_autorizacao_rh.strftime("%d/%m/%Y") if rd.data_autorizacao_rh else ""

    # Inserção das assinaturas
    adicionar_imagem_excel(ws, rd.assinatura_diretor, "Z55")
    adicionar_imagem_excel(ws, rd.assinatura_presidente, "B62")
    adicionar_imagem_excel(ws, rd.assinatura_rh, "Z62")
